...
- [x] 13. Дополнительное скругление иконок в настройках
  - [x] 13.1 Увеличить `borderRadius` иконок строк в `app/settings/index.tsx`
    - У контейнера иконки строки настроек (36×36) повысить `borderRadius` с `10` до `12` (squircle‑эффект, мягче по бокам)
    - У `<Image>` внутри (30×30) повысить `borderRadius` с `11` до `12` так, чтобы внутренний радиус был согласован с внешним
    - Оставить остальные стили (размер, фон, `overflow`) без изменений — это чистая визуальная правка
    - Проверить визуально, что иконки‑изображения (PNG из `assets/settings-icons/`) и иконки `Feather` выглядят однородно и не обрезаются
    - _Requirements: дополнительный UX‑запрос пользователя (иконки настроек ещё округлить по бокам)_
