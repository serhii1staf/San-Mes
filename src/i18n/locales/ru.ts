...
const ru: Record<string, string> = {
  ...
  // Подзаголовок и сноска намеренно говорят «для уведомлений», а не «все входы»:
  // единственная запись об устройстве на сервере — это push_tokens. Журнала входов
  // нет, токен авторизации не имеет состояния, и перечислять там нечего. Обещать
  // список сессий было бы неправдой.
  'devices.subtitle': 'Устройства, которые входили в этот аккаунт',
  // Сноска говорит именно то, что происходит, и не обещает большего: уведомления
  // отрубаются сразу, а выход происходит, когда устройство в следующий раз выйдет
  // в сеть. Иначе было бы непонятно, сработала ли кнопка.
  'devices.note': 'Уведомления на отключённом устройстве прекращаются сразу. Из аккаунта оно выйдет, когда в следующий раз подключится к сети.',
